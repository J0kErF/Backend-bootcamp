...
class Server:
    ...
    def sell_ticket(self, event):
        print(f"Attempting to sell ticket for {event}...")
        if event['event'] not in self.tickets_db or self.tickets_db[event['event']] <= 0:
        ...
        self.tickets_db[event['event']] -= event['tickets']

        with open(self.tickets_file, 'w') as f:
            json.dump(self.tickets_db, f)

        print(f"Sold 1 ticket for {event}. Remaining: {self.tickets_db[event['event']]}")
        self.current_concurrent_requests -= 1
        return True
    ...
    def process_request(self, event):
        ...
        self.current_concurrent_requests += 1
        print(f"Processing request for {event}...")
        self.sell_ticket(event)
        time.sleep(5)
        self.current_concurrent_requests -= 1
