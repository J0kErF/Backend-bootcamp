...
def decipher(text, keys):
    textlist = text.split()
    message = ''
    for key in keys:
        message+=textlist[int(key)][0]
    return message

def cipher(text, message):
    keys=[]
    textlist = text.split()
    for letter in message:
        for word in textlist:
            if word[0]==letter:
                keys.append(textlist.index(word))
                break
    return keys
...
print(decipher(text, key)) # should print

print(cipher(text, "apple is not good target to buy the stock of it"))
print(decipher(text, cipher(text, "apple is not good target to buy the stock of it")))